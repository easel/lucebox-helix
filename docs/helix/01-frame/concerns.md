...
|---------|--------|-------|------------|---------------|
| `typescript-bun` | library (language-runtime slot) | `area:api` `area:ui` `area:cli` | Server + UI written in TypeScript; Bun runtime for performance and unified tooling | Prefer Bun APIs over Node polyfills; pin bun version in lockfile; use Bun test runner |
| `react-nextjs` | library (frontend-framework slot) | `area:ui` | Management UI is a web app served locally; Next.js provides SSR + API routes in a single deployable | Use App Router; keep client components minimal; page components server-side by default |
...
| `deploy-target:local-appliance` | project-local assumption | `area:infra` | Lucebox ships as a pre-built box, not a cloud service; the deploy target is the physical hardware running a Linux-based OS | Software updates delivered as tested bundles; systemd services for inference daemon and UI server; no Kubernetes or cloud PaaS dependency at launch |

## Project Overrides

| Concern | Practice | Override | Authority |
|---------|----------|----------|-----------|
| `deploy-target:local-appliance` | (no shipped library concern — project-local) | Deploy to physical box via tested update bundle; not cloud-hosted | Needs ADR |
...
|------|--------------|--------|
| `frontend-framework` | `react-nextjs` | shipped default |
...
- `area:ui` — management UI (web, Next.js)
...
| `react-nextjs` SSR vs. real-time inference metrics | Inference metrics (tokens/second, queue depth) require live updates; use server-sent events or WebSocket from the inference API rather than SSR data fetching for the metrics panel |
