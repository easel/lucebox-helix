...
- **Desired outcome**: Zero cloud API calls when a developer uses any supported
  harness adapter. No adapter requires a cloud API key. A developer new to
...
ADAPT-01. The claude-code adapter locates the `claude` binary and launches it
configured to use the local Lucebox inference endpoint.

ADAPT-02. The codex adapter locates the `codex` binary and launches it
configured to use the local Lucebox inference endpoint.

ADAPT-03. The opencode adapter locates the `opencode` binary and launches it
configured to use the local Lucebox inference endpoint.

ADAPT-04. The hermes-agent adapter locates the `hermes-agent` binary and
launches it configured to use the local Lucebox inference endpoint.

ADAPT-05. The pi adapter locates the `pi` binary and launches it configured to
use the local Lucebox inference endpoint.

ADAPT-06. The openclaw adapter locates the `openclaw` binary and launches it
configured to use the local Lucebox inference endpoint.

ADAPT-07. Each adapter replaces the current process by handing off execution
to the underlying client binary — no Lucebox wrapper process remains running
after the underlying client binary is launched.
...
COMMON-01. Each adapter locates the client binary before proceeding; when the
...
COMMON-02. Each adapter sets the API base URL, API key, and model ID
environment variables for the target process before handing off execution.
...
CLAUDE-01. The claude-code adapter routes requests through an Anthropic
Messages API-compatible endpoint on the inference server so that the `claude`
CLI operates without code changes to the CLI or to the user's project
configuration.
...
- **Latency**: Adapter startup overhead (binary lookup + process handoff) adds
  ≤100ms to the first invocation of any supported harness adapter.
...
- [TODO: create story] US-XXX — Developer routes claude-code to local inference
  using the Lucebox harness adapter
- [TODO: create story] US-XXX — Developer routes codex to local inference
  using the Lucebox harness adapter
...
- [TODO: create story] US-XXX — Developer routes hermes-agent to local
  inference using the Lucebox harness adapter
- [TODO: create story] US-XXX — Developer routes pi to local inference using
  the Lucebox harness adapter
...
- **Underlying binary not installed**: The adapter's binary lookup fails before
  process handoff; the adapter exits immediately with a message naming the
  missing binary and indicating it must be installed separately. No partial
  environment setup occurs.
- **Inference server not running**: The adapter hands off to the underlying
  binary successfully; the error is surfaced by the underlying client tool when
  it attempts its first request. The adapter does not pre-validate server
  reachability.
...
- **Multiple concurrent adapter invocations**: Each invocation is independent;
  because adapters are stateless and hand off execution to the underlying
  process, concurrent invocations cannot interfere with each other.
...
- **Claude-code adapter with a non-Anthropic model ID active**: The
...
- 100% of supported adapters (claude-code, codex, opencode, hermes-agent, pi,
  ...
  running.
- Time from invoking any supported harness adapter to the underlying tool's
  first prompt is ≤60 seconds on a clean install (excluding server cold-start).
...
- Adapters are not responsible for informing the user whether the inference
  server is healthy; that responsibility belongs to the host compatibility check
  capability and the inference server itself.
